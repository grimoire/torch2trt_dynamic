...
def convert_tensor_getitem(ctx):
    ...
    for index, s in enumerate(new_slices):
        dim_shape_trt = tensor_trt_get_shape_trt(ctx.network, input_trt, index,
                                                 1, 1)
        ...
        if isinstance(s, slice):
            ...
            if start < 0:
                start_trt = ctx.network.add_elementwise(
                    dim_shape_trt, trt_(ctx.network, start),
            ...
            if size < 0:
                ...
                size_trt = ctx.network.add_elementwise(
                    dim_shape_trt, trt_(ctx.network, s.stop),
                    trt.ElementWiseOperation.SUM).get_output(0)
                size_trt = ctx.network.add_elementwise(
                    size_trt, start_trt,
            ...
            if s < 0:
                need_dynamic_input = True
                start_trt = ctx.network.add_elementwise(
                    dim_shape_trt, trt_(ctx.network, s),
                    trt.ElementWiseOperation.SUM).get_output(0)
