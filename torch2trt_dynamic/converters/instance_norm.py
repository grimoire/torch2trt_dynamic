...
def _add_scale_1d2d3d(network,
    ...
    if not support_dynamic_shape:
        if ndim != 3:
            ...
            layer.reshape_dims = (x_trt.shape[0], x_trt.shape[1], -1
                                  )  # NCH -> NCHW
            y_trt = layer.get_output(0)
    else:
        if ndim != 4:
            layer = network.add_shuffle(y_trt)
            layer.reshape_dims = (x_trt.shape[0], x_trt.shape[1],
                                  x_trt.shape[2], -1)  # NCH -> NCHW
            y_trt = layer.get_output(0)
...
def convert_instance_norm(ctx):
    ...
    if not use_input_stats:
        ...
        new_input_trt, shape_trt = _reshape_1d2d3d(
            ctx.network, input_trt)  # reshape if dim!=4
