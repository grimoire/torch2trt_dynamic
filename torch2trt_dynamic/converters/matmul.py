...
def convert_matmul(ctx):
    ...
    need_reshape = (len(input_a.shape) > 2 and len(input_b.shape) == 1) or (
        len(input_b.shape) > 2 and len(input_a.shape) == 1)
    ...
    if need_reshape:
        ...
        matmul_dim_trt = slice_shape_trt(ctx.network, old_a_shape_trt,
                                         len(input_a.shape) - 1)
        ...
        if len(input_a.shape) > 1:
            prefix_a_trt = slice_shape_trt(ctx.network, old_a_shape_trt,
                                           len(input_a.shape) - 2, 1)
        else:
            prefix_a_trt = None

        if len(input_b.shape) > 1:
            prefix_b_trt = slice_shape_trt(ctx.network, old_b_shape_trt,
                                           len(input_b.shape) - 1, 1)
        else:
            prefix_b_trt = None

        if len(input_a.shape) > 2:
            prefix_shape_trt = slice_shape_trt(ctx.network, old_a_shape_trt, 0,
                                               len(input_a.shape) - 2)
        ...
        if len(input_b.shape) > 2:
            if prefix_shape_trt is None:
                prefix_shape_trt = slice_shape_trt(ctx.network,
                                                   old_b_shape_trt, 0,
                                                   len(input_b.shape) - 2)
            ...
            layer.first_transpose = tuple([len(input_b.shape) - 2] +
                                          list(range(len(input_b.shape) - 2)) +
                                          [len(input_b.shape) - 1])
